...
Sia $v$ il nodo da inserire nell'albero $T$, si colora il nodo $v$ di \textbf{rosso} e si segue l'algoritmo di inserimento standard per i BST.
...
Per ovviare a questa violazione, si introduce una procedura ausiliaria chiamata fixDelete che si occupa di ribilanciare la black-height, essa viene chiamata sul nodo che fisicamente sostitusce il nodo appena eliminato 
...
\begin{enumerate}
    ...
    In questa situazione, vengono ricolorati $x$ e da $f$ di rosso, ora si procede esegue un controllo su $p[x]$.
    \begin{itemize}
...
\newpage
\subsubsection{Split}
L'operazione di \texttt{split}$(T, v)$ divide l'albero $T$ in due alberi $T_L$ e $T_R$ rispetto alla chiave del nodo $v$, quindi $T_L$ conterrà tutte le chiavi piu piccole di $v$, mentre $T_R$ tutte quelle maggiori di $v$ . La procedura avviene in due fasi:
\begin{enumerate}
...
\begin{proof}
    ...
    \begin{proof}
        ...
        L'accumulato $T_{L,i}$ è il risultato delle join effettuate con sottoalberi aventi rank minore o uguale a $S_i$ e siccome la funzione join come abbiamo visto preserva il rank possiamo concluedere che $rank(S_i) \geq rank(T_{L,i})$
    \end{proof} 
