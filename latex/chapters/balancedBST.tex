...
\begin{description}
    ...
    \begin{figure}[h] 
        ...
        \caption{Rotazione semplice destra (Caso SS).}
    ...
    \begin{figure}[h] 
        ...
        \caption{Rotazione doppia sinistra-destra (Caso SD).}
...
\begin{itemize}
    ...
    Il procedimento è simmetrico, ma questa volta si discende lungo il cammino dei figli sinistri di $S_2$ fino a trovare un nodo $x$ con $rank(x) = rank(S_1)$. Il nodo $i$ ( colorato di rosso) sostituirà $x$, prendendo $S_1$ come figlio sinistro e $x$ come figlio destro, poi si ricolora $x$ e $S_1$ di \textbf{nero}
\end{itemize}
...
\begin{enumerate}
    ...
    \begin{itemize}
        \item Se $v$ discende dal figlio sinistro di $p$, allora $p$ e il sottoalbero $p.right$ appartengono alla parte destra. Si esegue: \\ $T_R = \text{join}(p.right, p, T_R)$.
        \item Se $v$ discende dal figlio destro di $p$, allora $p$ e il sottoalbero $p.left$ appartengono alla parte sinistra. Si esegue: \\ $T_L = \text{join}(T_L, p, p.left)$.
    \end{itemize}
\end{enumerate}

\begin{proof}
    Nella dimostrazione ci concentreremo sull'accolumatore $T_L$ ma il ragionamento rimane invariato per $T_R$.\\
    Ad ogni passo $i$ se il cammino devia a destra uniremo quanto accumulato fino ad ora $T_{L,i}$ con un nuovo sottoalbero $S_i$, cosi da creare l'accumulatore $T_{L,i+1}$

    Denominiamo con $h_i = rank(T_{L,i})$ il rank dell'accumulato dopo l'i-esimo join e osserviamo che:
    \begin{itemize}
        \item per le proprietà dei Red-Black Trees il $rank(S_i) \geq  h$, poiche si troverà in un livello piu in alto
        \item Il risultato del join avra come rank o lo stesso $rank(S_i)$ o $rank(S_i)+1$, ne consegue che $h_{i+1} \approx rank(S_i)$
    \end{itemize}
    ...
    Poiché:
    \begin{itemize}
        \item $h_0 \geq 0$ è l'altezza del nodo di partenza;
