...
\chapter{Il Problema della Connettività Dinamica su foreste}
\labch{connectivita}

Il problema della connettività dinamica consiste nel mantenere aggiornate le informazioni 
sulla struttura di un grafo soggetto a modifiche topologiche repentine. Nel caso specifico delle 
foreste, l'obiettivo è la gestione di una collezione di alberi disgiunti tramite le operazioni di 
link e cut.\\

\begin{definition}
    Una foresta è un grafo aciclico non orientato
\end{definition}

Per gestire queste operazioni in modo efficiente, Henzinger e King 
\cite{HenzingerKing1999} introducono gli Euler Tour Trees (ETT). 
Questa struttura dati a differenza di altre strutture che operano 
direttamente sui nodi, si basa sulla linearizzazione dell'albero 
...
Il ciclo euleriano è strettamente connesso con la struttura dell'albero; ad esempio si osserva che la sequenza evidenziata (A,B), \textbf{(B,D), (D,B), (B,E), (E,B)}, (B,A), (A,C), (C,A) rappresenta il sottoalbero radicato in B

\begin{kaobox}[frametitle = Claim]
    Sia $S$ la sequenza di archi che descrive 
    il tour euleriamo per ogni nodo $v$ il 
    sottoalbero radicato in $v$ è rappresentato dall'intervallo 
    contiguo che va dal primo arco che ha $(v,\_)$ fino all'ultimo arco che contiene $(\_,v)$    
...
interno del ciclo euleriano il primo nodo del ciclo, per farlo seguiamo i seguenti passaggi:
...
\subsubsection{Cut}
L'operazione \textbf{cut(u,v)} permette di separare un albero in due alberi distinti. Eliminando l'arco $(u,v)$ creiamo due nuovi alberi radicati rispettivamente in $u$ e $v$, per farlo seguiamo i seguenti passaggi:
\begin{enumerate}
    \item rerooting(T,v)
    \item si cerca la sequenza $(u,v)$ e $(v,u)$ e si separa in A che conterrà tutto ciò che c'era prima di $(u,v)$, B che conterrà $(u,v)$ e $(v,u)$ e tutto ciò che c'è tra i due e C che contiene ciò che c'è dopo $(v,u)$
...
\end{algorithm}
 ...
 \begin{remark}
    Tutte queste implementazioni delle operazioni però hanno costo proporzionale alla dimensione del ciclo euleriano, dunque il loro costo sarà $O(n)$
 \end{remark}

Introduciamo una struttura dati che ci permetterà di eseguire queste operazioni in maniera molto piu efficente.
